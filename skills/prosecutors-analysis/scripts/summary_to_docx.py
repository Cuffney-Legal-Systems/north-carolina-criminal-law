#!/usr/bin/env python3
"""
summary_to_docx.py — render a prosecutorial-summary working draft (markdown)
into the delivered .docx, preserving the conventions in
reference/summary-template.md.

    python3 summary_to_docx.py <input.md> <output.docx>

Handles: fenced blocks (header block / closing notice), `>` case-critical
flags, ## and ### headings, pipe tables (real Word tables with a repeating
header row), `-` bullets, and inline **bold** / *italic* / `code`.
...
CONTENT_IN = 6.5                       # 8.5" letter - 2 x 1" margins
ELEM_COLS = [0.38, 2.32, 0.95, 2.85]   # #, Element, Status, Proof
BL_COLS = [2.25, 1.65, 2.60]           # Charge, Probable cause, Recommendation
...
def add_table(doc, rows):
    widths = ELEM_COLS if len(rows[0]) == 4 else BL_COLS
    ...
    for ri, cells in enumerate(rows):
        ...
        for ci, txt in enumerate(cells):
            ...
            if ri == 0:
                ...
                run.bold, run.font.name, run.font.size = True, BODY, Pt(10)
                shade(cell._tc, "E8E8E8")
            else:
                emit(par, txt, size=10)
...
def new_doc():
    ...
    for name, size, caps in (("Heading 1", 13, True), ("Heading 2", 11.5, False)):
        st = doc.styles[name]
        st.font.name, st.font.size = BODY, Pt(size)
        st.font.bold, st.font.color.rgb = True, RGBColor(0, 0, 0)
        st.font.all_caps = caps
        st.paragraph_format.space_before = Pt(14 if caps else 11)
    ...
    page_field(f)
    for r in f.runs:
        r.font.name, r.font.size = BODY, Pt(9)
...
def convert(md_path, out_path):
    ...
    while i < len(lines):
        ...
        if line.startswith(">"):                          # case-critical flag
            buf = []
            ...
            shade(p._p, "F7E9E9")
            border(p, ["left"], size="18", color="A03030")
            continue
...
if __name__ == "__main__":
    if len(sys.argv) != 3:
        sys.exit("usage: summary_to_docx.py <in.md> <out.docx>")
